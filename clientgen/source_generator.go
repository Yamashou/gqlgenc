...
func (r *SourceGenerator) NewResponseField(selection ast.Selection, typeName string) *ResponseField {
	...
	case *ast.Field:
		...
		case fieldsResponseFields.IsStructType():
			...
			baseType = types.NewNamed(
				types.NewTypeName(0, r.config.GQLGencConfig.Package.Pkg(), typeName, nil),
				structType,
				nil,
			)
		default:
	...
	case *ast.FragmentSpread:
		...
		baseType := types.NewNamed(
			types.NewTypeName(0, r.config.GQLGencConfig.Package.Pkg(), templates.ToGo(selection.Name), nil),
	...
	case *ast.InlineFragment:
		...
		if hasFragmentSpread {
			...
			typ := types.NewNamed(
				types.NewTypeName(0, r.config.GQLGencConfig.Package.Pkg(), name, nil),
				structType,
				nil,
			)
