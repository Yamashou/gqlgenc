...
func RenderTemplate(cfg *config.Config, fragments []*Fragment, operations []*Operation, operationResponses []*OperationResponse, structSources []*StructSource) error {
	genGettersGenerator := &GenGettersGenerator{
		ClientPackageName: cfg.GQLGencConfig.Package.Package,
	}
	if err := templates.Render(templates.Options{
		PackageName: cfg.GQLGencConfig.Package.Package,
		Filename:    cfg.GQLGencConfig.Package.Filename,
		Template:    template,
		Data: map[string]any{
			"Fragment":          fragments,
			"Operation":         operations,
			"OperationResponse": operationResponses,
			"GenerateClient":    cfg.GQLGencConfig.Package,
		...
		return fmt.Errorf("%s generating failed: %w", cfg.GQLGencConfig.Package.Filename, err)
	}
